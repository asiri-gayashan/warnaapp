...
import 'package:warna_app/presentation/institute/controllers/student_detail_controller.dart';
...
class _StudentDetailPageState extends State<StudentDetailPage> {
  late StudentDetailController _controller;

  @override
  void initState() {
    super.initState();
    // toMap() produces flat keys — fromJson expects the same flat shape
    // since StudentModel.fromJson reads flat keys directly (no nesting needed
    // beyond 'users'). We normalise back to the API shape here.
    final studentModel = StudentModel.fromJson(_normaliseMap(widget.student));
    _controller = StudentDetailController(student: studentModel);
                          ...
                          children: _controller.classes.map((cls) {
                            return UpcomingClassListTile(
                              ...
                              onTap: () => Navigator.push(
                                context,
                                MaterialPageRoute(
                                  builder: (context) => ClassDetailPage(classItemDetails: cls),
                                ),
                              ),
                            );
                          }).toList(),
