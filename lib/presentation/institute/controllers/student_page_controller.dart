...
class StudentPageController extends ChangeNotifier {
  ...
  // ── Current page items ───────────────────────────────────────
  List<StudentModel> get currentPageItems {
  ...
  Future<bool> fetchStudents() async {
    try {
      ...
      final response =
          await _dio.get("/students/institute/${user?["id"]}");
      // debugPrint("Fetch Students Response: ${response.data}");
      ...
      if (response.data['success'] == true &&
        ...
        _allStudents = data
            .map((json) =>
                StudentModel.fromJson(json as Map<String, dynamic>))
            .toList();
        _currentPage = 0;
        notifyListeners();
        return true;
